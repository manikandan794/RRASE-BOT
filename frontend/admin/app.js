(function () {
  if (!window.RraseDashboard.requireAnyRole(["admin", "principal"])) return;
  ...
  async function renderUsers() {
    content.innerHTML = "";
    content.appendChild(el("h2", { class: "h5 mb-3", text: "Users" }));

    const form = el("form", { class: "row g-2 mb-4" });
    const emailInput = el("input", { class: "form-control form-control-sm", placeholder: "Email" });
    const nameInput = el("input", { class: "form-control form-control-sm", placeholder: "Full name" });
    const passInput = el("input", { class: "form-control form-control-sm", placeholder: "Password", type: "password" });
    const roleSelect = el("select", { class: "form-select form-select-sm" });
    ["faculty", "admin", "principal"].forEach((r) => roleSelect.appendChild(el("option", { value: r, text: r })));
    [emailInput, nameInput, passInput, roleSelect].forEach((i) => {
      const col = el("div", { class: "col-md-2" });
      col.appendChild(i);
      form.appendChild(col);
    });
    const addBtn = el("button", { class: "btn btn-sm btn-primary", type: "submit", text: "Create staff account" });
    const btnCol = el("div", { class: "col-md-3" });
    btnCol.appendChild(addBtn);
    form.appendChild(btnCol);
    content.appendChild(form);

    const table = el("table", { class: "table table-sm bg-white" });
    table.innerHTML = "<thead><tr><th>Email</th><th>Name</th><th>Roles</th><th>Active</th><th></th></tr></thead>";
    const tbody = el("tbody");
    table.appendChild(tbody);
    content.appendChild(table);
    ...
    async function load() {
      tbody.innerHTML = "";
      const users = await window.RraseAPI.request("/users");
      users.forEach((u) => {
        ...
        toggleBtn.addEventListener("click", async () => {
          await window.RraseAPI.request(`/users/${u.id}`, { method: "PATCH", body: { is_active: !u.is_active } });
          load();
        });
        const td = el("td");
        td.appendChild(toggleBtn);
        tbody.appendChild(el("tr", {}, [
          el("td", { text: u.email }), el("td", { text: u.full_name }),
          el("td", { text: u.roles.join(", ") }), el("td", { text: u.is_active ? "Yes" : "No" }), td,
        ]));
      });
    }
    ...
    form.addEventListener("submit", async (e) => {
      e.preventDefault();
      try {
        await window.RraseAPI.request("/users/staff", {
          method: "POST",
          body: { email: emailInput.value, full_name: nameInput.value, password: passInput.value, role: roleSelect.value },
        });
        emailInput.value = ""; nameInput.value = ""; passInput.value = "";
        load();
      } catch (err) {
        alert(err.message);
      }
